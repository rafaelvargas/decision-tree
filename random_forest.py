...
class RandomForest:
    def __init__(self, number_of_trees: int, classification_attribute: str, random_state: int = 42):
    ...
    def construct(self, dataset: pd.DataFrame):
        ...
        for b in bootstraps:
            decision_tree = DecisionTree(classification_attribute=self.classification_attribute)
            decision_tree.construct(b)
